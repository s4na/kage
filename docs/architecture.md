...
- `kage-overlay`: backend boundary plus directory-merge fallback, deletion metadata, layout validation, idempotent unmount, and gated overlayfs detection.
...
The directory-merge backend exports a selected Git tree into `lower/` and presents a mutable `merged/` directory. This is a fallback for unprivileged development and testing. Commit-back does not commit this directory directly; it refreshes `upper/`, records deletions in `.kage/deleted`, then constructs a synthetic Git tree from parent tree + upper mutations.
