...
CMD='set -eux; if command -v apt-get >/dev/null 2>&1; then apt-get update; apt-get install -y fuse3 libfuse3-dev pkg-config build-essential attr util-linux fuse-overlayfs; fi; rustup component add rustfmt clippy; cargo fmt --all -- --check; cargo clippy --workspace --all-targets --all-features -- -D warnings; cargo test --workspace --all-features; KAGE_TEST_ROFS=1 cargo test --workspace --all-features -- --nocapture; KAGE_TEST_OVERLAY=1 cargo test --workspace --all-features -- --nocapture; KAGE_TEST_ROFS=1 KAGE_TEST_OVERLAY=1 cargo test --workspace --all-features -- --nocapture; KAGE_TEST_ROFS=1 KAGE_TEST_OVERLAY=1 KAGE_TEST_RUNTIME=1 cargo test --workspace --all-features -- --nocapture'
...
echo "== privileged kage rofs+overlay verification =="
echo "image: $IMAGE"
