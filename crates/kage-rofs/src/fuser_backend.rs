...
impl FuserRofs {
    ...
    fn path_for(&self, ino: INodeNo) -> Option<PathBuf> {
        self.inodes.lock().unwrap().path(ino.0)
    }
...
impl Filesystem for FuserRofs {
    ...
    fn lookup(&self, _req: &Request, parent: INodeNo, name: &OsStr, reply: ReplyEntry) {
        ...
        match self.view.lookup(&path) {
            Ok(meta) => {
                let ino = INodeNo(self.ino_for(path));
    ...
    fn readdir(
        ...
        for entry in entries {
            let entry_ino = INodeNo(self.ino_for(entry.path.clone()));
...
impl InodeTable {
    fn new() -> Self {
        ...
        by_ino.insert(INodeNo::ROOT.0, PathBuf::new());
        by_path.insert(PathBuf::new(), INodeNo::ROOT.0);
        Self {
            by_ino,
            by_path,
